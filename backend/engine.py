...
class prediction:
    ...
    def execute(company):
       ...
       # plt.show()
        ...
        tstep = 30
        # since we are looking 30 timesteps back, we can start start looping over only after 30th record in our training set
        data = []
        ...
        plt.savefig('D:/project/nsap/home/static/home/img/' + fileName, format='jpeg')
    ...
    def insertResult(company, fileName, nrms, pv):
        conn = connection.connectDatabase()
        cur = connection.getCur(conn)
        cur.execute("insert into result(company,filename,nrmse,predicted_value) values (%s,%s,%s,%s)",(company, fileName,float(nrms), float(pv)))
        conn.commit()
        cur.close()
